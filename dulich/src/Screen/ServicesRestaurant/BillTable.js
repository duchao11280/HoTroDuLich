...
import { getTablesHaveBooked } from '../../networking/restaurantnetworking'

const BillTable = ({ navigation }) => {
  const [listBookedTable, setListBookedTable] = useState([])
  ...
  const getBookedTable = async () => {
    setLoading(true)
    const user = await getUserID();
    getTablesHaveBooked(user)
      .then((response) => { setListBookedTable(response.data) })
  ...
  return (
    <KeyboardAwareScrollView style={{ flex: 1 }}>
      <Appbar.Header statusBarHeight={20}>
        ...
        <Appbar.Content title="Bàn được đặt" />
      </Appbar.Header>
              ...
              renderItem={({ item, index }) => {
                return (
                  <View style={styles.result}>
                    ...
                    <Text style={styles.textinfo}>Mã phòng: {item.tableID} </Text>
                    <Text style={styles.textinfo}>Tên phòng {item.tableName} </Text>
